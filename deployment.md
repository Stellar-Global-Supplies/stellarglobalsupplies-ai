...
**Tavily Search**
1. Go to **tavily.com** → Sign up for free plan (1,000 queries/month)
2. Dashboard → API Keys → Copy your API key
3. Save as `BRAVE_API_KEY` (starts with `tvly-`)
...
| `BRAVE_API_KEY` | From Step 2 (Tavily API key) |
...
5. Test the **Web** toggle — asks a question and check it searches via Tavily
...
- Tavily free tier allows 1,000 queries/month
...
| `BRAVE_API_KEY` | ✅ | Tavily Search API key |
